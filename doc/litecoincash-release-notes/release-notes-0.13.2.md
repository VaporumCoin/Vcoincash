LitecoinCash Core version 0.13.2 is now available from:

  <https://download.litecoinca.sh/litecoincash-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a litecoincash
...
ECDSA signatures inside LitecoinCash transactions now use validation using
...
Previous versions of LitecoinCash Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of LitecoinCash Core was to raise the default minimum
relay fee.

LitecoinCash Core 0.13.2 will have a strict maximum size on the mempool. The
...
LitecoinCash Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running litecoincashd without having to do any manual configuration.
...
LitecoinCash Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. LitecoinCash Core will relay transactions with
...
entirely in LitecoinCash Core 0.13, and it is at this time undecided whether the
...
LitecoinCash Core automatically creates a hidden service to listen on, without
manual configuration. LitecoinCash Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if LitecoinCash Core is listening, and
...
LitecoinCashd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals LitecoinCash
Core to use floating fees. By default, LitecoinCash Core will use floating
fees.
...
(default: `0.0002` LCC/kB).

At all times, LitecoinCash Core will cap fees at `-maxtxfee=<x>` (default:
0.10) LCC.
Furthermore, LitecoinCash Core will never create transactions paying less than
...
`litecoincash.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /litecoincashrpc>
        ProxyPass http://127.0.0.1:52457/
        ProxyPassReverse http://127.0.0.1:52457/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass litecoincashd rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< litecoincashrpc:<password>
    </Location>
...
Restarting litecoincashd will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in litecoincash.conf
...
litecoincash-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/litecoincash-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The LitecoinCash Core code base has
...
- `litecoincash-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `litecoincash-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the LitecoinCash scripting system that in combination with
...
LitecoinCash transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of LitecoinCash Core.
...
The two phases are distinct in the LitecoinCash-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming LitecoinCash wallet
  software more difficult and which seriously complicates the design of smart
  contracts for LitecoinCash.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each LitecoinCash block need to be stored by nodes in order to validate future
...
- **Increased security for multisig:** LitecoinCash addresses (both P2PKH addresses
  ...
  believed to be provided by LitecoinCash's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of LitecoinCash's current ledger (called the UTXO set),
  ...
  note that LitecoinCash Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  LitecoinCash users to individually opt-in to almost any change in the LitecoinCash
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and LitecoinCash Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the LitecoinCash
client to LitecoinCash Core 0.13.2 to signal for segwit support. When a
...
Since LitecoinCash Core 0.10.0, nodes have defaulted to only relaying and
...
- `litecoincash-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `litecoincash-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
