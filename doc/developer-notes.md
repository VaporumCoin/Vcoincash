...
Run with the -testnet option to run with "play litecoincash" on the test network, if you
...
```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_litecoincash
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_litecoincash --log_level=test_suite
$ valgrind -v --leak-check=full src/litecoincashd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_litecoincash.coverage/index.html`.
```
...
- ThreadRPCServer : Remote procedure call handler, listens on port 52457 for connections and services them.

- BitcoinMiner : Generates litecoincash (if wallet is enabled).
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `litecoincash-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `litecoincash-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of LitecoinCash Core, the wallet was always
